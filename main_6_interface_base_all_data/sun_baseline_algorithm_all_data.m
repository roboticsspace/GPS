...
function [Baseline_mCERIM_set,Baseline_mCERIM_DOF] = sun_baseline_algorithm_all_data(gpssolve,first_one_third_end_time,last_one_third_start_time,which_part_trial)
    ...
    for mt=1:length(match_time_platform.match_time)
        ...
        if ~isempty(match_time_platform.match_time(mt).p2) && ~isempty(match_time_platform.match_time(mt).p3)...,
                ...
                %summation
                match_time_platform.match_time(mt).Base_mCERIM = m_2 + m_3 + m_4;
                match_time_platform.match_time(mt).DOF = length(match_time_platform.match_time(mt).p2) +...,
                    length(match_time_platform.match_time(mt).p3) + length(match_time_platform.match_time(mt).p4);
        else
            continue;
        end
        ...
        Baseline_mCERIM_DOF = [Baseline_mCERIM_DOF,match_time_platform.match_time(mt).DOF];
    end
end    
