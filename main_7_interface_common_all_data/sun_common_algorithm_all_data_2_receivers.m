...
function [Common_mCERIM_set_2_receivers,Common_mCERIM_DOF_2_receivers] = sun_common_algorithm_all_data_2_receivers(gpssolve,first_one_third_end_time,last_one_third_start_time,which_part_trial)
    ...
    Common_mCERIM_set_2_receivers = [];
        Common_mCERIM_DOF_2_receivers = [];
    for mt=1:length(match_time_platform.match_time)
        ...
        elseif ~isempty(match_time_platform.match_time(mt).p2)
            ...
            %Nav
            [max_length,max_one]=max([length(N4),length(N2)]);
            if max_one == 1
                Nav=N4;
            else
                Nav=N2;
        ...
        elseif ~isempty(match_time_platform.match_time(mt).p3)                
            ...
            %Nav
            [max_length,max_one]=max([length(N4),length(N3)]);
            if max_one == 1
                Nav=N4;
            else
                Nav=N3;
        ...
        %Baseline mCERIM cal
        Common_mCERIM_set_2_receivers = [Common_mCERIM_set_2_receivers,mCERIM];
        Common_mCERIM_DOF_2_receivers = [Common_mCERIM_DOF_2_receivers, match_time_platform.match_time(mt).DOF];
    end
end    